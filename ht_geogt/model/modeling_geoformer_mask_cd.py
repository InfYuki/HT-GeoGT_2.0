...
#111
class GeoformerCenterNodeAttention(nn.Module):
    """
    只对中心节点应用注意力机制的Geoformer注意力层
    中心节点是指在EnhancedData中通过中心路径等算法更新过'x'的节点
    """

    def __init__(self, config, *args, **kwargs) -> None:
        super(GeoformerCenterNodeAttention, self).__init__(*args, **kwargs)
    ...
    def forward(
        ...
        # 识别中心节点 - 假设中心节点的特征在x的前11维和后11维不同
        # 在EnhancedData.py中，中心节点的特征被替换为[inv_features, equ_features]
        # 而非中心节点的特征是[original_x, original_x]的重复

        # 创建中心节点掩码 - 检测前11维和后11维是否相同
        B, N, F = x.shape
        half_dim = F // 2
        first_half = x[:, :, :half_dim]  # (B, N, F/2)
        second_half = x[:, :, half_dim:]  # (B, N, F/2)

        # 计算前后两半特征的差异
        feature_diff = torch.abs(first_half - second_half).sum(dim=-1)  # (B, N)
        center_node_mask = feature_diff > 1e-6  # 差异大于阈值的被视为中心节点
        ...
        # 应用掩码，只保留中心节点的注意力
        center_node_mask = repeat(
            center_node_mask, "b n -> (b h) n", h=self.num_heads
        )  # (BH, N)

        # 创建掩码，只允许中心节点接收注意力
        attn_mask = center_node_mask.unsqueeze(-1)  # (BH, N, 1)
        attn_weights = attn_weights.masked_fill(~attn_mask, 0.0)  # 非中心节点的注意力权重设为0
        ...
        # 对非中心节点，保持原始特征
        center_node_mask = center_node_mask.reshape(B, -1, N)[:, 0, :]  # (B, N)
        attn = torch.where(
            center_node_mask.unsqueeze(-1),  # (B, N, 1)
            attn,  # 中心节点使用注意力结果
            x  # 非中心节点保持原始特征
        )
...
class GeoformerAttnBlock(nn.Module):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        self.self_attn = GeoformerCenterNodeAttention(config)
