...
from ht_geogt.datasets import QM9, Molecule3D
from ht_geogt.model.collating_geoformer import GeoformerDataCollator
from ht_geogt.utils import MissingLabelException, make_splits

from ht_geogt.datasets import QM9, QM9Edge, Molecule3D

import os
from ht_geogt.datasets.qm9_enhanced import QM9Enhanced
...
class DataModule(LightningDataModule):
    ...
    # 添加新的数据集准备方法
    def _prepare_QM9Edge_dataset(self):
        self.dataset = QM9Edge(
            root=self.hparams["dataset_root"],
            dataset_arg=self.hparams["dataset_arg"],
        )
        idx_train, idx_val, idx_test = make_splits(
    ...
    def _prepare_QM9Enhanced_dataset(self):
        # 检查是否有增强数据文件
        #enhanced_data_path = os.path.join(self.hparams["dataset_root"], "qm9_enhanced_test.pt")

        self.dataset = QM9Enhanced(
            root=self.hparams["dataset_root"],
            dataset_arg=self.hparams["dataset_arg"],
        )
